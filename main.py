import logging
import asyncio
from telegram import ReplyKeyboardMarkup,WebAppInfo, ReplyKeyboardRemove, MenuButtonWebApp,KeyboardButton,Update, InlineKeyboardButton, InlineKeyboardMarkup
...
# user = User(name="San Bin Hoque", age = 20, email="epti060@gmail.com")
# user.save()

CAR_TYPE, CAR_COLOR, CAR_MILEAGE_DECISION, CAR_MILEAGE, PHOTO, SUMMARY = range(6)
web_app_url = "https://san-bin-hoque.netlify.app/"
...
async def start(update : Update, context : ContextTypes.DEFAULT_TYPE) -> int:
    reply_kayboard = [['Sedan', 'SUV', 'Sports', 'Electric']]
    button = [
        [MenuButtonWebApp("Open Game👹'",web_app=WebAppInfo(url=web_app_url))]
    ]
    await update.message.reply_text(f'<b>Hello {update.effective_chat.id},</b>,\nWelcome to the Car Sales Listing Bot!\n'
        'Let\'s get some details about the car you\'re selling.\n'
        'What is your car type?', parse_mode="HTML",reply_markup=ReplyKeyboardMarkup(button,one_time_keyboard=True, resize_keyboard=True))
...
async def test(update: Update, context : ContextTypes.DEFAULT_TYPE)-> int:
    update_obj = update
    print("update_obj", update)

    return 1

async def set_web_app(app):
...
def main() -> None:
    TOKEN = "7349809582:AAEvwgNtLm-i3lY2J9wyBZu3hsIGzBeEenc"
